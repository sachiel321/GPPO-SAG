...
class DelayHead(nn.Module):
    ...
    def forward(self, embedding, delay: Optional[torch.Tensor] = None):
        ...
        delay_encode = torch.nn.functional.one_hot(delay.long().detach(), self.delay_dim).float()
...
class QueuedHead(nn.Module):
    ...
    def forward(self, embedding, queued=None):
        ...
        queued_one_hot = torch.nn.functional.one_hot(queued.long().detach(), self.queued_dim).float()
        ...
        if selected_units is not None and selected_units_num is not None:  # train
            ...
            for i in range(max(seq_len, 1)):
                ...
                if self.whole_cfg.model.entity_reduce_type == 'selected_units_num' or 'attention' in self.whole_cfg.model.entity_reduce_type:
                    ...
                    new_selected_units_one_hot[torch.arange(bs).to(end_flag.device)[~end_flag], selected_units[:, i][~end_flag], :] = 1
                    if self.whole_cfg.model.entity_reduce_type == 'selected_units_num':
                        selected_units_emebedding = (key_embeddings * new_selected_units_one_hot).sum(dim=1).detach()
                        ...
                        ae = autoregressive_embedding + self.attention_pool(key_embeddings.detach(), mask=new_selected_units_one_hot.detach())
                    elif self.whole_cfg.model.entity_reduce_type == 'attention_pool_add_num':
                        ae = autoregressive_embedding + self.attention_pool(key_embeddings.detach(), num=new_selected_units_one_hot.detach().sum(dim=1).squeeze(dim=1),
                                                                            mask=new_selected_units_one_hot.detach(),)
                    selected_units_one_hot = new_selected_units_one_hot.clone()
                else:
                    ae = ae + key_embeddings[torch.arange(bs),
                                             selected_units[:, i]] * (i + 1 < selected_units_num).detach().unsqueeze(1)
            ...
            for i in range(self.max_select_num):
                ...
                if self.whole_cfg.model.entity_reduce_type == 'selected_units_num' or 'attention' in self.whole_cfg.model.entity_reduce_type:
                    selected_units_one_hot[torch.arange(bs)[~end_flag], result[~end_flag], :] = 1
                    if self.whole_cfg.model.entity_reduce_type == 'selected_units_num':
                        selected_units_emebedding = (key_embeddings * selected_units_one_hot).sum(dim=1).detach()
                        ...
                        ae = autoregressive_embedding + self.attention_pool(key_embeddings.detach(),
                                                                                mask=selected_units_one_hot.detach())
                    elif self.whole_cfg.model.entity_reduce_type == 'attention_pool_add_num':
                        ae = autoregressive_embedding + self.attention_pool(key_embeddings.detach(),
                                                                                num=selected_units_one_hot.detach().sum(
                                                                                    dim=1).squeeze(dim=1),
                                                                                mask=selected_units_one_hot.detach(), )
                else:
                    ae = ae + key_embeddings[torch.arange(bs), result].detach() * ~end_flag.detach().unsqueeze(dim=1)
                if end_flag.all():
                    break
            if self.extra_units:
