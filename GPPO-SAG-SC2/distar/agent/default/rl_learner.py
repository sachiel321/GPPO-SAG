...
from .rl_training.rl_loss_cov import ReinforcementLoss
...
class RLLearner(BaseLearner):
    ...
    def _train(self, data):
        with self._timer:
            ...
            log_vars, action_type_loss, other_loss = self._loss.compute_loss(model_output)
        ...
        with self._timer:
            ...
            # action_type_loss.backward(create_graph=True)

            # for k, param in self._model.policy.action_type_head.named_parameters():
            #     if k == 'action_fc.layer2.0.weight':
            #         action_type_gradient = param.grad.clone()
            ...
            for k, param in self._model.policy.action_type_head.named_parameters():
                if k == 'action_fc.layer2.0.weight':
                    other_type_gradient = param.grad# - action_type_gradient
                temp = param.grad
                print(temp)
            self._optimizer.step()
