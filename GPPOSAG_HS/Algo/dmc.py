...
def learn(position, actor_models, model, batch, optimizer, loss_func, flags, frames, lock, writter, grad_clip, use_factor=False):
    ...
    with lock:
        ...
        # for head_type in ['action_type', 'target_card', 'target_entity', 'target_position']:
        for head_type in ['action_type', 'target_entity', 'target_position']:
            ...
            if head_type == 'action_type':
                ...
                # winrate = torch.nonzero(torch.sum(reward,dim=0)+1).shape[0]/reward.shape[1]
