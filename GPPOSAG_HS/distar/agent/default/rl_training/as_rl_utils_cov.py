from distar.agent.default.model.head import action_type_head
...
def policy_gradient_loss(baseline_value, reward, target_action_log_probs_dict, clipped_rhos_dict, mask,
    ...
    for head_type in ['action_type', 'delay', 'queued', 'selected_units',  'target_unit',
        ...
        if head_type != 'action_type':
            other_policy_gradient_loss += (policy_gradient_loss * head_weights_dict[head_type])
        else:
            action_type_gradient_loss = policy_gradient_loss * head_weights_dict[head_type]
        policy_gradient_loss_dict[head_type] = policy_gradient_loss.item()

    policy_gradient_loss_dict['total'] = other_policy_gradient_loss.item()

    return action_type_gradient_loss, other_policy_gradient_loss, policy_gradient_loss_dict


def upgo_loss(baseline_value, reward, target_action_log_probs_dict, clipped_rhos_dict, mask, head_weights_dict):
    action_type_upgo_loss = 0.
    other_upgo_loss = 0.
    upgo_loss_dict = {}
    for head_type in ['action_type', 'delay', 'queued', 'selected_units',  'target_unit',
                      'target_location']:
        ...
        if head_type != 'action_type':
            other_upgo_loss += (upgo_loss * head_weights_dict[head_type])
        else:
            action_type_upgo_loss = upgo_loss * head_weights_dict[head_type]
        upgo_loss_dict['upgo/' + head_type] = upgo_loss.item()
    upgo_loss_dict['upgo/total'] = other_upgo_loss.item()
    return action_type_upgo_loss, other_upgo_loss, upgo_loss_dict


def entropy_loss(target_policy_probs_dict, target_policy_log_probs_dict, mask, head_weights_dict):
    action_type_entropy_loss = 0
    other_entropy_loss = 0
    entropy_dict = {}
    for head_type in ['action_type', 'delay', 'queued', 'selected_units',  'target_unit',
                      'target_location']:
        ...
        if head_type != 'action_type':
            other_entropy_loss += (-entropy * head_weights_dict[head_type])
        else:
            action_type_entropy_loss = -entropy * head_weights_dict[head_type]
    entropy_dict['entropy/total'] = other_entropy_loss.item()
    return action_type_entropy_loss, other_entropy_loss, entropy_dict
...
def kl_loss(target_policy_log_probs_dict, teacher_policy_logits_dict, mask, game_steps,
    ...
    for head_type in ['action_type', 'queued', 'delay', 'selected_units',  'target_unit',
        ...
        kl_loss = kl.mean()
        if head_type != 'action_type':
            other_kl_loss += (kl_loss * head_weights_dict[head_type])
        else:
            action_type_kl_loss = kl_loss * head_weights_dict[head_type]
        kl_loss_dict['kl/' + head_type] = kl_loss.item()
    kl_loss_dict['kl/total'] = other_kl_loss.item()
    return action_type_kl_loss, other_kl_loss, action_type_kl_loss, kl_loss_dict
