...
from .as_rl_utils_cov import td_lambda_loss,entropy_loss,policy_gradient_loss,upgo_loss,kl_loss,dapo_loss
...
class ReinforcementLoss:
    ...
    def compute_loss(self, inputs):
        ...
        # ====================
        action_type_policy_gradient_loss = 0
        other_policy_gradient_loss = 0
        ...
        for field, baseline in baseline_values_dict.items():
            ...
            field_action_type_policy_gradient_loss, field_other_policy_gradient_loss, field_policy_gradient_loss_dict = \
            ...
            other_policy_gradient_loss += self.loss_weights.pg[field] * field_other_policy_gradient_loss
            action_type_policy_gradient_loss += self.loss_weights.pg[field] * field_action_type_policy_gradient_loss
        ...
        # upgo loss
        # ===========
        action_type_upgo_loss, other_upgo_loss, upgo_loss_dict = upgo_loss(
        ...
        other_upgo_loss *= self.loss_weights.upgo.winloss
        action_type_upgo_loss *= self.loss_weights.upgo.winloss
        ...
        # entropy loss
        # ============
        action_type_entropy_loss, other_entropy_loss, entropy_dict = \
            entropy_loss(target_policy_probs_dict, target_policy_log_probs_dict, masks_dict,
                         head_weights_dict=self.entropy_head_weights)

        other_entropy_loss *= self.loss_weights.entropy
        action_type_entropy_loss *= self.loss_weights.entropy
        ...
        action_type_kl_loss, other_kl_loss, action_type_kl_loss, kl_loss_dict = \
        ...
        other_kl_loss *= self.loss_weights.kl
        ...
        if self.only_update_value:
            ...
            total_loss = other_policy_gradient_loss + \
                         other_upgo_loss + \
                         total_critic_loss + \
                         other_entropy_loss + \
                         other_kl_loss + \
                         total_dapo_loss + action_type_policy_gradient_loss + action_type_upgo_loss + action_type_kl_loss + action_type_entropy_loss
        action_type_loss = action_type_policy_gradient_loss + action_type_upgo_loss + action_type_kl_loss + action_type_entropy_loss
        other_loss = other_policy_gradient_loss + \
                        other_upgo_loss + \
                        total_critic_loss + \
                        other_entropy_loss + \
                        other_kl_loss
        loss_info_dict['total_loss'] = total_loss
        return loss_info_dict, action_type_loss, other_loss
