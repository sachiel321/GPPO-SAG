...
from .as_rl_utils import td_lambda_loss,entropy_loss,policy_gradient_loss,upgo_loss,kl_loss,nd_loss
...
class ReinforcementLoss:
    ...
    def compute_loss(self, inputs, head_type, factor):
        ...
        with torch.no_grad():
            ...
            clipped_rhos = rhos.clamp(1 - self.ratio_clip, 1 + self.ratio_clip)# .clamp_(max=1)
        ...
        loss_info_dict['rho/' + head_type] = rhos.mean().item()
        ...
        #     masks_dict['actions_mask'], head_type,factor)
        ...
        # total_nd_loss, action_type_nd_loss, nd_loss_dict = \
        #     nd_loss(target_policy_probs_dict, target_policy_log_probs_dict, teacher_policy_logits_dict, masks_dict, game_steps,
        #             action_type_kl_steps=self.action_type_kl_steps, head_type=head_type)
        # total_nd_loss *= self.loss_weights.kl
        # action_type_nd_loss *= self.loss_weights.action_type_kl


        # loss_info_dict.update(nd_loss_dict)
