...
from .rl_training.rl_loss_clip_rb import ReinforcementLoss
...
class RLLearner(BaseLearner):
    def __init__(self, cfg, *args):
        ...
        self._use_factor = cfg.learner.use_factor
    ...
    def _setup_optimizer(self):
        ...
        # self._optimizer = Adam([{'params': self.model.policy.parameters(), 'lr': self._whole_cfg.learner.learning_rate, 'betas':(0,0.99), 'eps':1e-5},
        #                         {'params': self.model.value_networks.parameters(), 'lr': self._whole_cfg.learner.learning_rate, 'betas':(0,0.99), 'eps':1e-5}])
    ...
    def _train(self, data):
        with self._timer:
            ...
            with torch.no_grad():
                model_output = self._model.rl_learner_forward(**copy.deepcopy(data))
        ...
        T, B = model_output['action_log_prob']['action_type'].size()
        self._factor = torch.ones((T, B), dtype=torch.float32, requires_grad=False).to(model_output['action_log_prob']['action_type'].device)
        self._factor_teacher = torch.ones_like(self._factor, requires_grad=False)
        self._factor_clip = torch.ones_like(self._factor, requires_grad=False)
        
        '''
        each agent update
        '''
        head_list = ['action_type', 'delay','queued', 'target_unit', 'selected_units','target_location']
        with self._timer:
            for head_type in head_list:

                model_output = self._model.rl_learner_forward(**copy.deepcopy(data))
                behaviour_actions_logprob = model_output['action_log_prob'][head_type] # behaviour policy logprob
                if self._whole_cfg.learner.use_dapo:
                    model_output['successive_logit'] = data['successive_logit']
                temp_log_vars = self._loss.compute_loss(model_output, head_type, self._factor_clip, self._factor_teacher)

                log_vars.update(temp_log_vars)

                if head_type == 'selected_units':
                    loss = temp_log_vars['total_loss'] * 0.01
                else:
                ...
                if self._use_factor:
                    with torch.no_grad():
                        new_model_output = self.model.rl_learner_forward(**copy.deepcopy(data))
                        self._factor_clip, self._factor, self._factor_teacher = self._loss.clip_factor(new_model_output, head_type, self._factor, self._factor_teacher)

                    # self._factor = self._factor * clipped_temp_rho.detach()
                    # self._factor = self._factor * temp_rho.detach()
        ...
        del self._factor, self._factor_teacher, self._factor_clip
    ...
    def register_stats(self) -> None:
        ...
        for k1 in ['winloss', 'build_order', 'built_unit', 'effect', 'upgrade', 'battle', 'upgo', 'kl', 'entropy','rho']:
