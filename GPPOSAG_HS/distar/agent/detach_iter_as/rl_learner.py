...
class RLLearner(BaseLearner):
    ...
    def _train(self, data):
        ...
        self._factor = torch.ones((T, B), dtype=torch.float32).to(model_output['action_log_prob']['action_type'].device)
        ...
        with self._timer:
            for head_type in head_list:
                ...
                model_output = self._model.rl_learner_forward(**copy.deepcopy(data))
                old_actions_logprob = model_output['action_log_prob'][head_type]
                if self._whole_cfg.learner.use_dapo:
                    model_output['successive_logit'] = data['successive_logit']
                temp_log_vars = self._loss.compute_loss(model_output, head_type, self._factor)
                ...
                if self._use_factor:
                    with torch.no_grad():
                        ...
                        actions = new_model_output['action'][head_type]
                        # compute target log_probs, probs(for entropy,kl), target_action_log_probs, log_rhos(for pg_loss,upgo_loss)
                        pi_target = torch.distributions.Categorical(logits=new_model_output['target_logit'][head_type])
                        new_actions_logprob = pi_target.log_prob(actions)

                    if head_type == 'selected_units':
                        old_actions_logprob.masked_fill_(~model_output['mask']['selected_units_mask'], 0)
                        old_actions_logprob = old_actions_logprob.sum(-1) * 0.01

                        new_actions_logprob.masked_fill_(~new_model_output['mask']['selected_units_mask'], 0)
                        new_actions_logprob = new_actions_logprob.sum(-1) * 0.01  
                    temp_rho = new_actions_logprob - old_actions_logprob
                    self._factor = self._factor*torch.exp(temp_rho).detach()
