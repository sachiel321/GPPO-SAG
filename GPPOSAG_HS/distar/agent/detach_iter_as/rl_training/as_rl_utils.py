...
def upgo_loss(baseline_value, reward, target_action_log_probs_dict, clipped_rhos_dict, mask, head_type,factor):
    upgo_loss_dict = {}
    ...
    if head_type in ['action_type','delay']:
        upgo_loss = - advantages * target_action_log_probs * factor
    else:
        upgo_loss = - advantages * target_action_log_probs * factor * mask[head_type]
...
def dapo_loss(target_policy_log_probs_dict, successive_policy_logits_dict, mask, game_steps,
    ...
    kl_loss_dict['battle/total'] = kl_loss.item()
